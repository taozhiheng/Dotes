...
//	private float height = 0;
	...
	private int outerSelectedColor = 0xff8cbad8;
	private int outerErrorColor = 0xff901032;
	private int dotColor = 0xffd9d9d9;
	private int outerDotColor = 0xff929292;
    ...
    /**
     * 根据每个point选中情况：选中、错误、正常，绘制同心圆
     * 绘制两个point之间及最后一个point与触摸非point处连线，三角箭头
     * */
		...
		for (int i = 0; i < mPoints.length; i++) {
			for (int j = 0; j < mPoints[i].length; j++) {
				...
				if (p.state == Point.STATE_CHECK) {
					selectedPaint.setColor(outerSelectedColor);
					...
					normalPaint.setColor(outerDotColor);
	 ...
	 * 
	 * @param p 待检查的point
	 * @return int 状态 0-不包含 1-只有这个point 2-不是最后一个point
	 */
...
//			if ((dx > 1 || dy > 1) && (dx != 2 * dy) && (dy != 2 * dx)) {
	...
	public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("drag", "isTouch in TouchEvent:"+isTouch);
...
//			}
		...
		if (isFinish) {
			if (this.sPoints.size() == 1) {
				...
				clearPassword();
                if(sPoints.size()>0)
				    Toast.makeText(this.getContext(), "密码长度必须为4~9",
    ...
    /**
     *@param time time时间后标记所有point为错误状态
     */
	...
	/**
	 * @param mCompleteListener　监听器，在手指抬起时回调
	 */
